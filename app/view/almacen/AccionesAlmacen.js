...
Ext.define('tiendarepuestos.view.almacen.AccionesAlmacen', {
    ...
    onClickEliminarAlmacen:function(btn){
        me = this;
        Ext.Ajax.request({
                url: tiendarepuestos.util.Rutas.almacenEliminar, 
    ...
    onClickNuevoSeccion:function(btn)
    {
     ...
     onClickEliminarSeccion:function(btn){
        me = this;
        Ext.Ajax.request({
                url: tiendarepuestos.util.Rutas.seccionAlmacenEliminar, 
                params: { id          : btn.getWidgetRecord().get('id')},
                success: function(response){
                    var _error = Ext.JSON.decode(response.responseText);
                    if(_error.error!=0){
