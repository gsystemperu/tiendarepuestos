Ext.define('tiendarepuestos.view.ventas.AccionesRegFacturaBoleta', {
    ...
    onClickBuscarProducto: function (btn) {
      if(Ext.ComponentQuery.query('#cboDatosCliente')[0].getValue()){
        var _win = Ext.create('tiendarepuestos.view.ventas.BuscarProductoFB', { 
    ...
    onClickIngresarCotizacion: function (btn) {
        var _win = Ext.create('tiendarepuestos.view.ventas.RegistrarCotizacion');
        _win.show(btn, function () {}, this);
    },
    ...
    onClickEliminarCliente:function(button, event, eOpts){
      ...
      Ext.MessageBox.confirm('Aviso','Desea eliminar al Cliente ?',function(btn){
        if(btn=='yes'){
          if (rec) {
             Ext.Ajax.request({
                 url :tiendarepuestos.util.Rutas.clienteEliminar,
                 params:{
    ...
    onClickEliminarCotizacion:function(button, event, eOpts){
      ...
      Ext.MessageBox.confirm('Aviso','Desea Anular la cotizacion ?',function(btn){
        if(btn=='yes'){
          if (rec) {
             Ext.Ajax.request({
                 url :tiendarepuestos.util.Rutas.cotizacionEliminar,
                 params:{
                   vIdCoti : rec.get('vid')
                 },
                 success:function(response){
    ...
    onClickNuevoCliente: function (btn) {
        var _win = Ext.create('tiendarepuestos.view.ventas.RegistrarCliente');
        _win.show(btn, function () {}, this);
    },
    ...
    onClickGuardarFacturaBoleta: function () {
       var _form =  Ext.ComponentQuery.query('#frmRegFacturaBoleta')[0];    //this.lookupReference('frmRegFacturaBoleta');
        if (_form.isValid()) {
            ...
            _form.submit({
                ...
                success: function (form, action) {
                    if(action.result.error!=0){
                        _form.reset();
                        _store.removeAll();
                        var objrpt = window.open( tiendarepuestos.util.Rutas.rptImprimirNota+ 'id='+ action.result.error, "", "width=700,height=900");
            ...
            tiendarepuestos.util.Util.showErrorMsg('error al guardar factura individual');
        }
    },
    ...
    onClickMantenimiento: function () {
        var _win = Ext.create('tiendarepuestos.view.ventas.Mantenimiento');
    },
    ...
    onClickGuardarCliente: function () {
        ...
        if (_form.isValid()) {
            ...
            tiendarepuestos.util.Util.showErrorMsg('Ingresar los datos para la cotización!');
        }
    },
            ...
            if( Ext.ComponentQuery.query('window')[1].objeto !=''){ // Si esta opcion de objeto no es vacio estamos es un modal de mantenimiento para los combos
                ...
                if (_form.isValid()) {
                    _form.submit({
                        ...
                        failure: function () {
                            tiendarepuestos.util.Util.showErrorMsg("Error al momento de grabar la informacion");
                        }
                    });
                } else {
                    tiendarepuestos.util.Util.showErrorMsg('Ingresar los datos para la cotización!');
    ...
    onClickBuscarRUCDatos:function(){
        ...
        tiendarepuestos.util.Util.obtenerDatosRUC(_rucper,_txtNombre,_txtDireccion,_form);
    },
    //Obtener datos del cliente por su RUC
    onClickBuscarRUCDatosSimple:function(){
        ...
        tiendarepuestos.util.Util.obtenerDatosRUCSimple(_rucper,_txtNombre,_txtDireccion,_form);
    },
    ...
    onClickGuardarProducto: function (btn) {
        ...
        if (_form.isValid()) {
            ...
            tiendarepuestos.util.Util.showErrorMsg('Ingresar los datos del producto');
        }
    },
    ...
    onClickBuscarCotizacionesAnteriores:function(btn){
        ...
        if(_codcliente)
            Ext.create('tiendarepuestos.view.ventas.CotizacionesClienteBuscar',{ codigo : _codcliente });
