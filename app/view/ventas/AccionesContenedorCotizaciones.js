Ext.define('tiendarepuestos.view.ventas.AccionesContenedorCotizaciones', {
    ...
    onClickImprimirPDFCotizacion: function (btn) {
        ...
        if (_record) {
            var objrpt = window.open( tiendarepuestos.util.Rutas.imprimirProforma+ 'id='+ _record.get('vid'), "", "width=700,height=900");
    ...
    onClickConfirmarCotizacion:function(btn){
      ...
      if (_record) {
         me.getView().mask('..... confirmando datos');
          Ext.Ajax.request({
            url :tiendarepuestos.util.Rutas.confirmarVentaCotizacion,
            params:{
    ...
    onClickVisorClienteCotizaciones: function (btn) {
       /* var _record = this.lookupReference('dgvVentas').getSelectionModel().getSelection()[0];
        if (_record) {
            ...
            var _win = Ext.create('tiendarepuestos.view.ventas.VisorClienteCotizacion');
...
*/
            var _win = Ext.create('tiendarepuestos.view.ventas.VisorProductoCotizacion');
            _win.show(btn, function () {}, this);
        /*} else {
            Ext.Msg.alert("Error", "Tiene que seleccionar un cliente");
            return false;
    ...
    onClickVisorVendedorCotizaciones: function (btn) {
        ...
        if (_record) {
            ...
            var _win = Ext.create('tiendarepuestos.view.ventas.VisorVendedorCotizacion');
            _win.show(btn, function () {}, this);
        /*} else {
            Ext.Msg.alert("Error", "Tiene que seleccionar un cliente");
            return false;
    ...
    onClickEnviarCotizacion:function(btn){
        ...
        if(_record){
          Ext.Ajax.request({
              url : tiendarepuestos.util.Rutas.cotizacionEnviarMail,
              params:{
                id :_record.get('vid')
              },
              success:function(response){
