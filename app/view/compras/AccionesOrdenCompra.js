Ext.define('tiendarepuestos.view.compras.AccionesOrdenCompra', {
    ...
    onClickIngresar: function () {
        Ext.create('tiendarepuestos.view.compras.IngresarOrdenCompra');
    },
    onClickBuscarProducto: function (btn) {
       if(Ext.ComponentQuery.query('#cboProveedoresf')[0].getValue()!=null){
        Ext.create('tiendarepuestos.view.almacen.ProductoBuscarOC',{
          proveedor : Ext.ComponentQuery.query('#cboProveedoresf')[0].getValue()
        });
      }else{
        tiendarepuestos.util.Util.showToast('Seleccionar al Proveedor');
      }
    },
    ...
    onClickGuardarOrdenCompra: function () {
        ...
        if (_form.isValid()) {
            ...
            tiendarepuestos.util.Util.showErrorMsg('Ingresar los datos necesarios!');
        }
    },
    //==============================================
    ...
    onClickFormularioProveedor: function (btn) {
         win = Ext.create('tiendarepuestos.view.almacen.FormProveedor', {
    ...
    onClickEditarOrdenCompra:function(button){
       rec = button.getWidgetRecord();
       _me  = this;
      if(rec.get('idestado')==4){ tiendarepuestos.util.Util.showToast('La OC esta anulada!');; return false; }
      if (rec) {
          try {
             me =  Ext.ComponentQuery.query('#wContenedorOrdenCompra')[0];    //this;
             l = me.getLayout();
            ...
            _store.removeAll();
            Ext.Ajax.request({
                url :tiendarepuestos.util.Rutas.ordenCompraBuscarDetalle,
                params:{
                  id :rec.get('id')
                },
                success:function(response){
    ...
    onClickAnularOrdenCompra:function(button){
      ...
      if (rec) {
          try {
            ...
            Ext.Ajax.request({
                url :tiendarepuestos.util.Rutas.ordenCompraAnular,
                params:{
                  id :rec.get('id')
                },
                success:function(response){
